...
# Item properties
@export var item_name: String = "blood"  # What material this gives
...
# References
@onready var sprite = $blood  # Updated to match your scene structure
...
func show_pickup_indicator():
	# Intense pulse effect for blood
	if sprite and not is_collecting:
		var tween = create_tween()
		tween.set_loops()
		# More dramatic pulsing for blood
		tween.tween_property(sprite, "scale", Vector2(1.15, 1.15), 0.25)
...
func show_collection_message():
	...
	match item_name:
		...
		"blood":
			message_label.add_theme_color_override("font_color", Color(1.0, 0.4, 0.4))  # Bright red
		"stone":
...
func play_collection_animation():
	...
	# Try to get the sprite node - check multiple possible names for blood
	var sprite_node = null
	if has_node("blood"):
		sprite_node = $blood
		print("Found blood sprite node")
	elif has_node("Blood"):
		sprite_node = $Blood
		print("Found Blood sprite node")
	...
	if sprite_node:
		...
		# Intense blood flash (dramatic red surge)
		var blood_flash = Color(2.0, 0.4, 0.4)  # Intense red flash
		tween.tween_property(sprite_node, "modulate", blood_flash, 0.06)  # Faster flash
		tween.tween_property(sprite_node, "modulate", Color.TRANSPARENT, 0.25).set_delay(0.06)
		
		# Dramatic blood splatter movement
		tween.tween_property(sprite_node, "position", original_position + Vector2(0, -20), 0.25).set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
		tween.tween_property(sprite_node, "scale", Vector2(1.4, 1.4), 0.12).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		tween.tween_property(sprite_node, "scale", Vector2(0.1, 0.1), 0.2).set_delay(0.12)
		
		# Aggressive rotation for blood splatter effect
		tween.tween_property(sprite_node, "rotation", deg_to_rad(270), 0.32)
...
func create_pickup_particles():
	# Intense blood splatter particle effect
	var particles = CPUParticles2D.new()
	get_parent().add_child(particles)
	particles.global_position = global_position
	particles.emitting = true
	particles.amount = 18  # More particles for dramatic blood effect
	particles.lifetime = 0.7  # Quick splatter
	particles.one_shot = true
	particles.initial_velocity_min = 50
	particles.initial_velocity_max = 90
	particles.spread = 360
	particles.gravity = Vector2(0, 80)  # Fast drop like blood droplets
	...
	match item_name:
		...
		"blood":
			particles.color = Color(1.0, 0.3, 0.3)  # Deep red blood particles
		"stone":
	...
	particles.angular_velocity_min = -120
	particles.angular_velocity_max = 120
...
func add_floating_effect():
	if sprite:
		# Slightly more agitated floating for blood
		var tween = create_tween()
		tween.set_loops()
		tween.tween_property(sprite, "position:y", sprite.position.y - 3, 1.8).set_trans(Tween.TRANS_SINE)
		tween.tween_property(sprite, "position:y", sprite.position.y + 3, 1.8).set_trans(Tween.TRANS_SINE)

func add_glow_effect():
	if sprite:
		# Pulsing red glow for blood
		var tween = create_tween()
		tween.set_loops()
		tween.tween_property(sprite, "modulate:a", 0.7, 1.3)  # More dramatic fade
		tween.tween_property(sprite, "modulate:a", 1.0, 1.3)

func add_shimmer_effect():
	# Subtle blood shimmer (like wet blood catching light)
	if sprite:
		var shimmer_tween = create_tween()
		shimmer_tween.set_loops()
		# Slightly more dramatic shimmer for blood
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(2.5), 2.8).set_trans(Tween.TRANS_SINE)
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(-2.5), 2.8).set_trans(Tween.TRANS_SINE)
...
static func spawn_item(scene_parent: Node, item_type: String, position: Vector2, min_amt: int = 1, max_amt: int = 10):
	...
	var item_scene = preload("res://Scenes/elments Scens/blood.tscn")  # Update path for blood scene
