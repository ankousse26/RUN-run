...
# Item properties
@export var item_name: String = "gold"   # What material this gives
...
# References
@onready var sprite = $gold  # Updated to match your scene structure
...
func show_collection_message():
	...
	match item_name:
		...
		"gold":
			message_label.add_theme_color_override("font_color", Color(1.0, 0.9, 0.3))  # Bright golden
		_:
...
func play_collection_animation():
	...
	# Try to get the sprite node - check multiple possible names for gold
	var sprite_node = null
	if has_node("gold"):
		sprite_node = $gold
		print("Found gold sprite node")
	elif has_node("Gold"):
		sprite_node = $Gold
		print("Found Gold sprite node")
	...
	if sprite_node:
		...
		# Enhanced golden flash (brighter for gold!)
		var bright_flash = original_modulate * 3.0  # Extra bright for gold
...
func create_pickup_particles():
	# Enhanced particle effect for gold (more particles = more precious!)
	var particles = CPUParticles2D.new()
	get_parent().add_child(particles)
	particles.global_position = global_position
	particles.emitting = true
	particles.amount = 20  # More particles for gold!
	particles.lifetime = 1.0  # Slightly longer for gold sparkle
	particles.one_shot = true
	particles.initial_velocity_min = 40
	particles.initial_velocity_max = 80
	particles.spread = 360
	particles.gravity = Vector2(0, 30)  # Slower fall for floating gold effect
	...
	match item_name:
		...
		"gold":
			particles.color = Color(1.0, 0.9, 0.3)  # Bright golden particles
		_:
...
func add_shimmer_effect():
	# Enhanced shimmer effect for gold
	if sprite:
		var shimmer_tween = create_tween()
		shimmer_tween.set_loops()
		# More noticeable shimmer for gold
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(3), 2.5).set_trans(Tween.TRANS_SINE)
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(-3), 2.5).set_trans(Tween.TRANS_SINE)
...
static func spawn_item(scene_parent: Node, item_type: String, position: Vector2, min_amt: int = 1, max_amt: int = 10):
	...
	var item_scene = preload("res://Scenes/elments Scens/gold.tscn")  # Update path for gold scene
