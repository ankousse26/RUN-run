...
# Item properties
@export var item_name: String = "crystal"  # What material this gives
...
# References
@onready var sprite = $crystal  # Updated to match your scene structure
...
func show_pickup_indicator():
	# Magical, ethereal pulse for crystal
	if sprite and not is_collecting:
		var tween = create_tween()
		tween.set_loops()
		# Elegant pulsing for crystal (feels magical)
		tween.tween_property(sprite, "scale", Vector2(1.12, 1.12), 0.35)
...
func show_collection_message():
	...
	match item_name:
		...
		"crystal":
			message_label.add_theme_color_override("font_color", Color(0.7, 0.9, 1.0))  # Magical blue
		"coal":
...
func play_collection_animation():
	...
	# Try to get the sprite node - check multiple possible names for crystal
	var sprite_node = null
	if has_node("crystal"):
		sprite_node = $crystal
		print("Found crystal sprite node")
	elif has_node("Crystal"):
		sprite_node = $Crystal
		print("Found Crystal sprite node")
	...
	if sprite_node:
		...
		# Magical crystal flash (prismatic rainbow effect)
		var crystal_flash = Color(1.5, 1.5, 1.8)  # Bright magical blue-white
		tween.tween_property(sprite_node, "modulate", crystal_flash, 0.08)
		tween.tween_property(sprite_node, "modulate", Color.TRANSPARENT, 0.25).set_delay(0.08)
		
		# Elegant, ethereal floating movement
		tween.tween_property(sprite_node, "position", original_position + Vector2(0, -35), 0.4).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		tween.tween_property(sprite_node, "scale", Vector2(1.5, 1.5), 0.18).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		tween.tween_property(sprite_node, "scale", Vector2(0.2, 0.2), 0.22).set_delay(0.18)
		
		# Magical spinning rotation
		tween.tween_property(sprite_node, "rotation", deg_to_rad(360), 0.4)
...
func create_pickup_particles():
	# Magical sparkle particle effect
	var particles = CPUParticles2D.new()
	get_parent().add_child(particles)
	particles.global_position = global_position
	particles.emitting = true
	particles.amount = 25  # More particles for magical effect
	particles.lifetime = 1.4  # Longer magical sparkle
	particles.one_shot = true
	particles.initial_velocity_min = 30
	particles.initial_velocity_max = 70
	particles.spread = 360
	particles.gravity = Vector2(0, -10)  # Slight upward drift for magical effect
	...
	match item_name:
		...
		"crystal":
			particles.color = Color(0.8, 0.9, 1.0)  # Magical sparkle blue
		"coal":
	...
	# Magical sparkle rotation
	particles.angular_velocity_min = -150
	particles.angular_velocity_max = 150
	
	# Extended cleanup for magical effect
	get_tree().create_timer(1.6).timeout.connect(particles.queue_free)

func add_floating_effect():
	if sprite:
		# Enhanced floating for crystal (more magical)
		var tween = create_tween()
		tween.set_loops()
		tween.tween_property(sprite, "position:y", sprite.position.y - 6, 2.2).set_trans(Tween.TRANS_SINE)
		tween.tween_property(sprite, "position:y", sprite.position.y + 6, 2.2).set_trans(Tween.TRANS_SINE)

func add_glow_effect():
	if sprite:
		# Enhanced magical glow for crystal
		var tween = create_tween()
		tween.set_loops()
		tween.tween_property(sprite, "modulate:a", 0.6, 1.2)  # More dramatic glow
		tween.tween_property(sprite, "modulate:a", 1.0, 1.2)

func add_shimmer_effect():
	# Enhanced magical shimmer for crystal
	if sprite:
		var shimmer_tween = create_tween()
		shimmer_tween.set_loops()
		# More dramatic rotation shimmer for crystal
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(4), 2.5).set_trans(Tween.TRANS_SINE)
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(-4), 2.5).set_trans(Tween.TRANS_SINE)
...
static func spawn_item(scene_parent: Node, item_type: String, position: Vector2, min_amt: int = 1, max_amt: int = 10):
	...
	var item_scene = preload("res://Scenes/elments Scens/crystal.tscn")  # Update path for crystal scene
