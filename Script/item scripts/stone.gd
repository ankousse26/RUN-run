...
# Item properties
@export var item_name: String = "stone"  # What material this gives
...
# References
@onready var sprite = $stone  # Updated to match your scene structure
...
func show_pickup_indicator():
	# Solid, steady pulse for stone
	if sprite and not is_collecting:
		var tween = create_tween()
		tween.set_loops()
		# More subtle pulsing for stone (feels more solid)
		tween.tween_property(sprite, "scale", Vector2(1.08, 1.08), 0.4)
...
func show_collection_message():
	...
	match item_name:
		...
		"stone":
			message_label.add_theme_color_override("font_color", Color(0.8, 0.8, 0.8))  # Light gray
		"rope":
...
func play_collection_animation():
	...
	# Try to get the sprite node - check multiple possible names for stone
	var sprite_node = null
	if has_node("stone"):
		sprite_node = $stone
		print("Found stone sprite node")
	elif has_node("Stone"):
		sprite_node = $Stone
		print("Found Stone sprite node")
	...
	if sprite_node:
		...
		# Solid stone flash (earthy gray glow)
		var stone_flash = Color(1.0, 1.0, 0.9)  # Warm stone flash
		tween.tween_property(sprite_node, "modulate", stone_flash, 0.08)
		tween.tween_property(sprite_node, "modulate", Color.TRANSPARENT, 0.25).set_delay(0.08)
		
		# Heavy, grounded movement (stone doesn't float much)
		tween.tween_property(sprite_node, "position", original_position + Vector2(0, -15), 0.35).set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
		...
		# Solid, steady rotation (stone is stable)
		tween.tween_property(sprite_node, "rotation", deg_to_rad(90), 0.35)
...
func create_pickup_particles():
	# Rocky dust particle effect
	var particles = CPUParticles2D.new()
	get_parent().add_child(particles)
	particles.global_position = global_position
	particles.emitting = true
	particles.amount = 14  # Moderate particles for stone dust
	particles.lifetime = 1.0  # Longer dust settling
	particles.one_shot = true
	particles.initial_velocity_min = 20
	particles.initial_velocity_max = 55
	particles.spread = 360
	particles.gravity = Vector2(0, 90)  # Heavy gravity - stone dust falls fast
	...
	match item_name:
		...
		"stone":
			particles.color = Color(0.7, 0.7, 0.7)  # Gray stone dust
		"rope":
	...
	# Stone dust settles slowly
	particles.angular_velocity_min = -30
	particles.angular_velocity_max = 30
...
func add_floating_effect():
	if sprite:
		# Subtle floating for stone (feels more grounded)
		var tween = create_tween()
		tween.set_loops()
		tween.tween_property(sprite, "position:y", sprite.position.y - 2, 2.8).set_trans(Tween.TRANS_SINE)
		tween.tween_property(sprite, "position:y", sprite.position.y + 2, 2.8).set_trans(Tween.TRANS_SINE)

func add_glow_effect():
	if sprite:
		# Very subtle glow for stone (stone doesn't glow much)
		var tween = create_tween()
		tween.set_loops()
		tween.tween_property(sprite, "modulate:a", 0.85, 2.0)  # Less dramatic
		tween.tween_property(sprite, "modulate:a", 1.0, 2.0)

func add_shimmer_effect():
	# Minimal shimmer for stone (stone is stable and solid)
	if sprite:
		var shimmer_tween = create_tween()
		shimmer_tween.set_loops()
		# Very subtle rotation for stone
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(1), 4.0).set_trans(Tween.TRANS_SINE)
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(-1), 4.0).set_trans(Tween.TRANS_SINE)
...
static func spawn_item(scene_parent: Node, item_type: String, position: Vector2, min_amt: int = 1, max_amt: int = 10):
	...
	var item_scene = preload("res://Scenes/elments Scens/stone.tscn")  # Update path for stone scene
