...
# Item properties
@export var item_name: String = "rope"   # What material this gives
...
# References
@onready var sprite = $rope  # Updated to match your scene structure
...
func setup_item_appearance():
	...
	match item_name:
		...
		"rope":
			sprite.modulate = Color(0.8, 0.7, 0.5)  # Warm natural tan
		"crystal":
...
func show_pickup_indicator():
	# Organic, flowing pulse for rope
	if sprite and not is_collecting:
		var tween = create_tween()
		tween.set_loops()
		# Flexible pulsing for rope (organic feel)
		tween.tween_property(sprite, "scale", Vector2(1.09, 1.09), 0.38)
		tween.tween_property(sprite, "scale", Vector2(1.0, 1.0), 0.38)
...
func show_collection_message():
	...
	match item_name:
		...
		"rope":
			message_label.add_theme_color_override("font_color", Color(1.0, 0.9, 0.6))  # Warm tan
		"crystal":
...
func play_collection_animation():
	...
	# Try to get the sprite node - check multiple possible names for rope
	var sprite_node = null
	if has_node("rope"):
		sprite_node = $rope
		print("Found rope sprite node")
	elif has_node("Rope"):
		sprite_node = $Rope
		print("Found Rope sprite node")
	...
	if sprite_node:
		...
		# Warm rope flash (natural fiber glow)
		var rope_flash = Color(1.2, 1.1, 0.8)  # Warm tan flash
		tween.tween_property(sprite_node, "modulate", rope_flash, 0.08)
		tween.tween_property(sprite_node, "modulate", Color.TRANSPARENT, 0.25).set_delay(0.08)
		
		# Flowing, organic movement (rope sways and flows)
		tween.tween_property(sprite_node, "position", original_position + Vector2(0, -22), 0.32).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		tween.tween_property(sprite_node, "scale", Vector2(1.25, 1.25), 0.16).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		tween.tween_property(sprite_node, "scale", Vector2(0.25, 0.25), 0.18).set_delay(0.16)
		
		# Flowing rotation (like rope unraveling)
		tween.tween_property(sprite_node, "rotation", deg_to_rad(150), 0.34)
...
func create_pickup_particles():
	# Fiber particle effect (like rope strands)
	var particles = CPUParticles2D.new()
	get_parent().add_child(particles)
	particles.global_position = global_position
	particles.emitting = true
	particles.amount = 13  # Moderate amount for fiber strands
	particles.lifetime = 1.1  # Fibers drift for a bit
	particles.one_shot = true
	particles.initial_velocity_min = 15
	particles.initial_velocity_max = 50
	particles.spread = 360
	particles.gravity = Vector2(0, 35)  # Light gravity - fibers drift gently
	...
	match item_name:
		...
		"rope":
			particles.color = Color(0.9, 0.8, 0.6)  # Natural fiber tan
		"crystal":
	...
	# Gentle fiber rotation
	particles.angular_velocity_min = -75
	particles.angular_velocity_max = 75
...
func add_floating_effect():
	if sprite:
		# Organic, flowing movement for rope
		var tween = create_tween()
		tween.set_loops()
		tween.tween_property(sprite, "position:y", sprite.position.y - 3.5, 2.3).set_trans(Tween.TRANS_SINE)
		tween.tween_property(sprite, "position:y", sprite.position.y + 3.5, 2.3).set_trans(Tween.TRANS_SINE)

func add_glow_effect():
	if sprite:
		# Natural glow for rope (warm and organic)
		var tween = create_tween()
		tween.set_loops()
		tween.tween_property(sprite, "modulate:a", 0.8, 1.6)  # Moderate natural glow
		tween.tween_property(sprite, "modulate:a", 1.0, 1.6)

func add_shimmer_effect():
	# Organic shimmer for rope (like fibers catching light)
	if sprite:
		var shimmer_tween = create_tween()
		shimmer_tween.set_loops()
		# Natural, flowing rotation shimmer
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(2.2), 3.2).set_trans(Tween.TRANS_SINE)
		shimmer_tween.tween_property(sprite, "rotation", deg_to_rad(-2.2), 3.2).set_trans(Tween.TRANS_SINE)
...
static func spawn_item(scene_parent: Node, item_type: String, position: Vector2, min_amt: int = 1, max_amt: int = 10):
	...
	var item_scene = preload("res://Scenes/elments Scens/rope.tscn")  # Update path for rope scene
